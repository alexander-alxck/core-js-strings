...
function removeFirstOccurrences(str, value) {
  ...
  return idx === -1
    ? str
    : str.slice(0, idx).concat(str.slice(idx + value.length));
}
...
function formatTime(minutes, seconds) {
  return `${minutes < 10 ? minutes.toString().padStart(2, '0') : minutes}:${seconds < 10 ? seconds.toString().padStart(2, '0') : seconds}`;
}
...
function isPalindrome(str) {
  const clearStr = str.replace(/[^A-Za-z]/gm, '');

  for (let i = 0; i < Math.floor(clearStr.length); i += 1) {
    if (
      clearStr[i].toLowerCase() !==
      clearStr[clearStr.length - i - 1].toLowerCase()
    ) {
      return false;
    }
  }
...
function findLongestWord(sentence) {
  ...
  arr
    .map((item) => ({ length: item.length, str: item }))
    .forEach((item) => {
